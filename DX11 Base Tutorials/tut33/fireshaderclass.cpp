...
bool FireShaderClass::Initialize(HWND hwnd) {
	...
	auto result = InitializeShader(hwnd, L"../../tut33/fire.hlsl", L"../../tut33/fire.hlsl");
	if (!result) {
		return false;
	}
...
bool FireShaderClass::Render(int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	...
	auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, fireTexture, noiseTexture, alphaTexture,
...
bool FireShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "FireVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	result = device->CreateBuffer(&distortionBufferDesc, NULL, &distortion_buffer_);
	if (FAILED(result)) {
		return false;
	}
...
bool FireShaderClass::SetShaderParameters(const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	...
	result = device_context->Map(noise_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
	...
	result = device_context->Map(distortion_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
