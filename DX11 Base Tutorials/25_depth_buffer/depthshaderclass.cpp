...
bool DepthShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"depth.hlsl", L"depth.hlsl");
  if (!result) {
    return false;
  }
...
bool DepthShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
  ...
  auto flag = D3D10_SHADER_ENABLE_STRICTNESS | D3D10_SHADER_DEBUG |
              D3D10_SHADER_SKIP_OPTIMIZATION;

  auto result =
      D3DCompileFromFile(vsFilename, NULL, NULL, "DepthVertexShader", "vs_5_0",
                         flag, 0, &vertexShaderBuffer, &errorMessage);
  if (FAILED(result)) {
    if (errorMessage) {
  ...
  result =
      D3DCompileFromFile(psFilename, NULL, NULL, "DepthPixelShader", "ps_5_0",
                         flag, 0, &pixelShaderBuffer, &errorMessage);
  if (FAILED(result)) {
    if (errorMessage) {
