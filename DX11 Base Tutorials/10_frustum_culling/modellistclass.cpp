...
bool ModelListClass::Initialize(int numModels) {
  ...
  // Go through all the models and randomly generate the model color and
  // position.
  for (int i = 0; i < model_count_; i++) {
    ...
    model_info_list_[i].positionX =
        (((float)rand() - (float)rand()) / RAND_MAX) * 10.0f;
    model_info_list_[i].positionY =
        (((float)rand() - (float)rand()) / RAND_MAX) * 10.0f;
    model_info_list_[i].positionZ =
...
int ModelListClass::GetModelCount() { return model_count_; }

void ModelListClass::GetData(int index, float &positionX, float &positionY,
                             float &positionZ, XMFLOAT4 &color) {
