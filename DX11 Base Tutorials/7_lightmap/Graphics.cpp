...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
  ...
  {
    lightmap_shader_ =
        (LightMapShaderClass *)_aligned_malloc(sizeof(LightMapShaderClass), 16);
    ...
    if (!result) {
      MessageBox(hwnd, L"Could not initialize the light map shader object.",
                 L"Error", MB_OK);
      return false;
    }
  }
...
void GraphicsClass::Render() {
  ...
  lightmap_shader_->Render(model_->GetIndexCount(), worldMatrix, viewMatrix,
