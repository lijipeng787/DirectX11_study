...
bool DeferredBuffersClass::Initialize(int textureWidth, int textureHeight,
  ...
  for (i = 0; i < BUFFER_COUNT; i++) {
    result = device->CreateRenderTargetView(m_renderTargetTextureArray[i],
  ...
  for (i = 0; i < BUFFER_COUNT; i++) {
    result = device->CreateShaderResourceView(m_renderTargetTextureArray[i],
...
void DeferredBuffersClass::SetRenderTargets() {
  ...
  device_context->OMSetRenderTargets(BUFFER_COUNT, m_renderTargetViewArray,
                                     depth_stencil_view_);
  device_context->RSSetViewports(1, &viewport_);
}

void DeferredBuffersClass::ClearRenderTargets(float red, float green,
...
DeferredBuffersClass::GetShaderResourceView(int view) {
