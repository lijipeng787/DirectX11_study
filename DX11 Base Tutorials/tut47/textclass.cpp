...
bool TextClass::Initialize(HWND hwnd, int screenWidth, int screenHeight,
  ...
  result = font_->Initialize(device, "../../tut47/data/fontdata.txt",
                             L"../../tut47/data/font.dds");
  if (!result) {
  ...
  result = UpdateSentence(sentence_1_, "Intersection: No", 20, 20, 1.0f, 0.0f,
                          0.0f, device_context);
  if (!result) {
    return false;
  }
...
bool TextClass::Render(const XMMATRIX &worldMatrix,
...
bool TextClass::UpdateSentence(SentenceType *sentence, char *text,
  ...
  result = device_context->Map(sentence->vertexBuffer, 0,
...
bool TextClass::RenderSentence(SentenceType *sentence,
  ...
  device_context->IASetVertexBuffers(0, 1, &sentence->vertexBuffer, &stride,
                                     &offset);

  device_context->IASetIndexBuffer(sentence->indexBuffer, DXGI_FORMAT_R32_UINT,
                                   0);
  ...
  result =
      font_shader_->Render(sentence->indexCount, worldMatrix, base_view_matrix_,
                           orthoMatrix, font_->GetTexture(), pixelColor);
  if (!result) {
    false;
  }
...
bool TextClass::SetIntersection(bool intersection,
  ...
  if (intersection) {
    ...
    result = UpdateSentence(sentence_1_, intersectionString, 20, 20, 0.0f, 1.0f,
                            0.0f, device_context);
  } else {
    strcpy_s(intersectionString, "Intersection: No");
    result = UpdateSentence(sentence_1_, intersectionString, 20, 20, 1.0f, 0.0f,
                            0.0f, device_context);
  }
