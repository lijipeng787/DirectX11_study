...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
...

  {
    ...
    result = model_->Initialize("../../tut47/data/sphere.txt",
                                L"../../tut47/data/blue.dds");
    if (!result) {
...

  {
    ...
    result = text_->Initialize(directx_device_->GetDeviceContext(), hwnd,
...

  {
    ...
    result = bitmap_->Initialize(screenWidth, screenHeight,
                                 L"../../tut47/data/mouse.dds", 32, 32);
    if (!result) {
...
void GraphicsClass::TestIntersection(int mouseX, int mouseY) {
  ...
  float pointX, pointY;
  XMMATRIX projectionMatrix, viewMatrix, inverseViewMatrix, worldMatrix,
      translateMatrix, inverseWorldMatrix;
  ...
  // Adjust the points using the projection matrix to account for the aspect
  ...
  direction.m128_f32[0] = (pointX * inverseViewMatrix.r[0].m128_f32[0]) +
                          (pointY * inverseViewMatrix.r[1].m128_f32[0]) +
                          inverseViewMatrix.r[2].m128_f32[0];
  direction.m128_f32[1] = (pointX * inverseViewMatrix.r[0].m128_f32[1]) +
                          (pointY * inverseViewMatrix.r[1].m128_f32[1]) +
                          inverseViewMatrix.r[2].m128_f32[1];
  direction.m128_f32[2] = (pointX * inverseViewMatrix.r[0].m128_f32[2]) +
                          (pointY * inverseViewMatrix.r[1].m128_f32[2]) +
                          inverseViewMatrix.r[2].m128_f32[2];
  ...
  // Now transform the ray origin and the ray direction from view space to world
  // space.
  ...
  XMFLOAT3 rayOriginCopy(rayOrigin.m128_f32[0], rayOrigin.m128_f32[1],
                         rayOrigin.m128_f32[2]);
  XMFLOAT3 rayDirectionCopy(rayDirection.m128_f32[0], rayDirection.m128_f32[1],
                            rayDirection.m128_f32[2]);
  intersect = RaySphereIntersect(rayOriginCopy, rayDirectionCopy, 1.0f);

  if (intersect == true) {
    // If it does intersect then set the intersection to "yes" in the text
    // string that is displayed to the screen.
...
bool GraphicsClass::RaySphereIntersect(const XMFLOAT3 &rayOrigin,
  ...
  a = (rayDirection.x * rayDirection.x) + (rayDirection.y * rayDirection.y) +
      (rayDirection.z * rayDirection.z);
  b = ((rayDirection.x * rayOrigin.x) + (rayDirection.y * rayOrigin.y) +
       (rayDirection.z * rayOrigin.z)) *
      2.0f;
  c = ((rayOrigin.x * rayOrigin.x) + (rayOrigin.y * rayOrigin.y) +
  ...
  // if discriminant is negative the picking ray missed the sphere, otherwise it
...
bool GraphicsClass::Render() {
  ...
  result = light_shader_->Render(model_->GetIndexCount(), worldMatrix,
  ...
  result = bitmap_->Render(mouse_x_, mouse_y_);
  if (!result) {
    return false;
  }
  result =
