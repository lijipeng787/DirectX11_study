...
bool FontShaderClass::Initialize(HWND hwnd) {
  ...
  result =
      InitializeShader(hwnd, L"../../tut47/font.vs", L"../../tut47/font.ps");
  if (!result) {
    return false;
  }
...
bool FontShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "FontVertexShader",
...
bool FontShaderClass::SetShaderParameters(const XMMATRIX &worldMatrix,
  ...
  result = device_context->Map(constant_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0,
                               &mappedResource);
  if (FAILED(result)) {
    return false;
  }
