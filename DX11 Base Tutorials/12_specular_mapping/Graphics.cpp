...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
...

  {
    ...
    result = model_->Initialize("data/cube.txt", L"data/stone02.dds",
  ...
  {
    specularmap_shader_ =
        (SpecMapShaderClass *)_aligned_malloc(sizeof(SpecMapShaderClass), 16);
    ...
    if (!result) {
      MessageBox(hwnd, L"Could not initialize the specular map shader object.",
                 L"Error", MB_OK);
      return false;
    }
  }
...
void GraphicsClass::Render() {
  ...
  specularmap_shader_->Render(
      ...
      model_->GetTextureArray(), light_->GetDirection(),
