...
bool GraphicsClass::RenderSceneToTexture() {
	...
	m_CubeModel->Render();
	result = depth_shader_->Render(m_CubeModel->GetIndexCount(), worldMatrix, lightViewMatrix, lightProjectionMatrix);
	if (!result) {
		return false;
	}
	...
	result = depth_shader_->Render(m_SphereModel->GetIndexCount(), worldMatrix, lightViewMatrix, lightProjectionMatrix);
	if (!result) {
		return false;
	}
	...
	result = depth_shader_->Render(m_GroundModel->GetIndexCount(), worldMatrix, lightViewMatrix, lightProjectionMatrix);
	if (!result) {
		return false;
	}
...
bool GraphicsClass::Render() {
	...
	result = m_ShadowShader->Render(
		...
		worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix, lightProjectionMatrix,
		m_GroundModel->GetTexture(), render_texture_->GetShaderResourceView(), light_->GetPosition(),
