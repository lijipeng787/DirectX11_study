...
bool FogShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"fog.hlsl", L"fog.hlsl");
  if (!result) {
    return false;
  }
...
void FogShaderClass::Shutdown() { ShutdownShader(); }

bool FogShaderClass::Render(int indexCount, const XMMATRIX &worldMatrix,
                            const XMMATRIX &viewMatrix,
                            const XMMATRIX &projectionMatrix,
                            ID3D11ShaderResourceView *texture, float fogStart,
                            float fogEnd) {
...
bool FogShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                      WCHAR *psFilename) {

  ID3D10Blob *errorMessage = nullptr;
  ID3D10Blob *vertexShaderBuffer = nullptr;

  auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "FogVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "FogPixelShader",
  ...
  result = device->CreateBuffer(&fogBufferDesc, NULL, &fog_buffer_);
  if (FAILED(result)) {
    return false;
  }
...
void FogShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                              HWND hwnd,
...
bool FogShaderClass::SetShaderParameters(const XMMATRIX &worldMatrix,
                                         const XMMATRIX &viewMatrix,
                                         const XMMATRIX &projectionMatrix,
  ...
  result = device_context->Map(fog_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0,
                               &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  device_context->Unmap(fog_buffer_, 0);

  buffer_number = 1;

  device_context->VSSetConstantBuffers(buffer_number, 1, &fog_buffer_);
