...
bool Bitmap::Initialize(WCHAR *texture_filename, UINT screen_width,
                        UINT screen_height, UINT bitmap_width,
                        UINT bitmap_height) {
...
void Bitmap::SetVertexShader(
    const D3D12_SHADER_BYTECODE &verte_shader_bitecode) {
...
RootSignaturePtr Bitmap::GetRootSignature() const { return root_signature_; }

PipelineStateObjectPtr Bitmap::GetPipelineStateObject() const {
  return pso_depth_disabled_;
}

DescriptorHeapPtr Bitmap::GetShaderRescourceView() const {
  return texture_->get_texture();
}

D3d12ResourcePtr Bitmap::GetConstantBuffer() const { return constant_buffer_; }

bool Bitmap::UpdateConstantBuffer(DirectX::XMMATRIX &world,
  ...
  range.End = 0;
  UINT8 *data_begin = 0;
  if (FAILED(constant_buffer_->Map(0, &range,
...
int Bitmap::GetIndexCount() { return index_count_; }
...
bool Bitmap::InitializeBuffers() {
  ...
  D3d12ResourcePtr upload_index_buffer = nullptr;
  if (FAILED(device->CreateCommittedResource(
          &CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
          D3D12_HEAP_FLAG_NONE,
...
bool Bitmap::InitializeGraphicsPipelineState() {
  ...
  if (FAILED(device->CreateGraphicsPipelineState(
          &pso_desc, IID_PPV_ARGS(&pso_depth_disabled_)))) {
    return false;
  }
...
bool Bitmap::InitializeRootSignature() {
  ...
  if (FAILED(D3D12Device::GetD3d12DeviceInstance()
                 ->GetD3d12Device()
...
bool Bitmap::LoadTexture(WCHAR *texture_filename) {
