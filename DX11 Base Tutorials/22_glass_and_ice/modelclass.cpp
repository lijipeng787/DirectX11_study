...
bool ModelClass::Initialize(char *modelFilename, WCHAR *colorTextureFilename,
  ...
  result = LoadTextures(colorTextureFilename, normalTextureFilename);
  if (!result) {
    return false;
  }
...
bool ModelClass::LoadTextures(WCHAR *colorTextureFilename,
                              WCHAR *normalTextureFilename) {
  ...
  auto result = texture_->Initialize(colorTextureFilename);
  if (!result) {
    return false;
  }

  normal_map_ = new TextureClass();
  if (!normal_map_) {
    return false;
  }

  result = normal_map_->Initialize(normalTextureFilename);
  if (!result) {
    return false;
  }
