...
bool FadeShaderClass::Initialize(HWND hwnd) {
	...
	auto result = InitializeShader(hwnd, L"../../tut28/fade.hlsl", L"../../tut28/fade.hlsl");
	if (!result) {
		return false;
	}
...
bool FadeShaderClass::Render(int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	...
	auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, fadeAmount);
	if (!result) {
		return false;
	}
...
bool FadeShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "FadeVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
