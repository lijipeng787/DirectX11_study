...
bool PbrShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  error = wcscpy_s(vsFilename, 128, L"./pbr.vs");
  if (error != 0) {
    return false;
  }
...
bool PbrShaderClass::Render(ID3D11DeviceContext *deviceContext, int indexCount,
  ...
  result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
                               projectionMatrix, diffuseTexture, normalMap,
                               rmTexture, lightDirection, cameraPosition);
  if (!result) {
    return false;
  }
...
bool PbrShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
  ...
  // Setup the description of the camera dynamic constant buffer that is in the
  // vertex shader.
  ...
  // Create the camera constant buffer pointer so we can access the vertex
...
bool PbrShaderClass::SetShaderParameters(
  ...
  result = deviceContext->Map(m_cameraBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Now set the camera constant buffer in the vertex shader with the updated
  // values.
...
void PbrShaderClass::RenderShader(ID3D11DeviceContext *deviceContext,
                                  int indexCount) {
