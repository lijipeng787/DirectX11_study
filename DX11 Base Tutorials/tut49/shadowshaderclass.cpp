...
bool ShadowShaderClass::Initialize(HWND hwnd) {
  ...
  result = InitializeShader(hwnd, L"../../tut49/shadow.vs",
                            L"../../tut49/shadow.ps");
  if (!result) {
    return false;
  }
...
bool ShadowShaderClass::Render(
    ...
    ID3D11ShaderResourceView *depthMapTexture, const XMFLOAT3 &lightDirection,
  ...
  result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix,
                               lightViewMatrix, lightProjectionMatrix, texture,
                               depthMapTexture, lightDirection, ambientColor,
                               diffuseColor);
  if (!result) {
    return false;
  }
...
bool ShadowShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "ShadowVertexShader",
