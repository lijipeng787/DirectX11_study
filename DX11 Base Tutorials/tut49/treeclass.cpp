...
bool TreeClass::Initialize(char *trunkModelFilename,
                           WCHAR *trunkTextureFilename, char *leafModelFilename,
                           WCHAR *leafTextureFilename, float scale) {
  bool result;
  ...
  // Initialize the vertex and index buffer that hold the geometry for the tree
  // trunk.
...
int TreeClass::GetTrunkIndexCount() { return m_trunkIndexCount; }

int TreeClass::GetLeafIndexCount() { return m_leafIndexCount; }
...
bool TreeClass::InitializeTrunkBuffers(float scale) {
  ...
  for (i = 0; i < vertex_count_; i++) {
    vertices[i].position =
        XMFLOAT3(model_[i].x * scale, model_[i].y * scale, model_[i].z * scale);
  ...
  result =
      device->CreateBuffer(&index_buffer_desc, &indexData, &m_trunkIndexBuffer);
  if (FAILED(result)) {
    return false;
  }
...
bool TreeClass::InitializeLeafBuffers(float scale) {
  ...
  result =
      device->CreateBuffer(&index_buffer_desc, &indexData, &m_leafIndexBuffer);
  if (FAILED(result)) {
    return false;
  }
...
void TreeClass::RenderTrunkBuffers(ID3D11DeviceContext *device_context) {
  ...
  device_context->IASetVertexBuffers(0, 1, &m_trunkVertexBuffer, &stride,
                                     &offset);
...
void TreeClass::RenderLeafBuffers(ID3D11DeviceContext *device_context) {
  ...
  device_context->IASetVertexBuffers(0, 1, &m_leafVertexBuffer, &stride,
                                     &offset);
