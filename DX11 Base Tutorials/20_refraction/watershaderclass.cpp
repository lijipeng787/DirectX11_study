...
bool WaterShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"water.hlsl", L"water.hlsl");
  if (!result) {
    return false;
  }
...
void WaterShaderClass::Shutdown() { ShutdownShader(); }

bool WaterShaderClass::Render(int indexCount, const XMMATRIX &worldMatrix,
  ...
  auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix,
                                    ...
                                    refractionTexture, normalTexture,
...
bool WaterShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                        WCHAR *psFilename) {

  ID3D10Blob *errorMessage = nullptr;
  ID3D10Blob *vertexShaderBuffer = nullptr;

  auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "WaterVertexShader",
  ...
  result =
      device->CreateBuffer(&reflectionBufferDesc, NULL, &reflection_buffer_);
  if (FAILED(result)) {
    return false;
  }
