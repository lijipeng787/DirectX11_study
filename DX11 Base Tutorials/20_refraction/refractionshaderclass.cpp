...
bool RefractionShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"refraction.hlsl", L"refraction.hlsl");
  if (!result) {
    return false;
  }
...
bool RefractionShaderClass::Render(
    int indexCount, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix,
    const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView *texture,
    const XMFLOAT3 &lightDirection, const XMFLOAT4 &ambientColor,
    const XMFLOAT4 &diffuseColor, const XMFLOAT4 &clipPlane) {

  auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix,
                                    texture, lightDirection, ambientColor,
...
bool RefractionShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                             WCHAR *psFilename) {
  ...
  auto result = D3DCompileFromFile(
      vsFilename, NULL, NULL, "RefractionVertexShader", "vs_5_0",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "RefractionPixelShader",
...
void RefractionShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                     HWND hwnd,
