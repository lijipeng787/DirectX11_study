						   ...
						   const XMMATRIX& baseViewMatrix )
{
	...
	result = m_Font->Initialize(device, "../../tut12/data/fontdata.txt", L"../../tut12/data/font.dds");
	if(!result)
	{
	...
	result = UpdateSentence(m_sentence1, "Hello", 100, 100, 1.0f, 1.0f, 1.0f, device_context);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Goodbye", 100, 200, 1.0f, 1.0f, 0.0f, device_context);
	if(!result)
	{
		return false;
	}
...
bool TextClass::Render( ID3D11DeviceContext* device_context, const XMMATRIX& worldMatrix, const XMMATRIX& orthoMatrix )
{
	...
	result = RenderSentence(device_context, m_sentence2, worldMatrix, orthoMatrix);
	if(!result)
	{
		return false;
	}
...
bool TextClass::RenderSentence( ID3D11DeviceContext* device_context, SentenceType* sentence, const XMMATRIX& worldMatrix, const XMMATRIX& orthoMatrix )
{
