...
bool VerticalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../../tut46/verticalblur.vs", L"../../tut46/verticalblur.ps");
	if(!result)
	{
		return false;
	}
...
bool VerticalBlurShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
...
bool VerticalBlurShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
												  const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float screenHeight)
{
	HRESULT result;
