...
bool GlowShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"glow.vs", L"glow.ps");
  if (!result) {
    return false;
  }
...
void GlowShaderClass::Shutdown() { ShutdownShader(); }

bool GlowShaderClass::Render(int indexCount, const XMMATRIX &worldMatrix,
  ...
  auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix,
                                    colorTexture, glowTexture, glowStrength);
  if (!result) {
    return false;
  }
...
bool GlowShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                       WCHAR *psFilename) {

  ID3D10Blob *errorMessage = nullptr;
  ID3D10Blob *vertexShaderBuffer = nullptr;

  auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "GlowVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "GlowPixelShader",
...
void GlowShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                               HWND hwnd,
...
bool GlowShaderClass::SetShaderParameters(
  ...
  result = device_context->Map(m_glowBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                               &mappedResource);
  if (FAILED(result)) {
    return false;
  }
