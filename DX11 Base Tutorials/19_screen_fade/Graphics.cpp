...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
...

  {
    ...
    result = bitmap_->Initialize(screenWidth, screenHeight, screenWidth,
                                 screenHeight);
    if (!result) {
...

  {
    ...
    fade_shader_ =
        (FadeShaderClass *)_aligned_malloc(sizeof(FadeShaderClass), 16);
    ...
    if (!result) {
      MessageBox(hwnd, L"Could not initialize the fade shader object.",
                 L"Error", MB_OK);
      return false;
    }
  }
...
bool GraphicsClass::Frame() {
  ...
  if (!is_fade_done_) {
    ...
    // While the time goes on increase the fade in amount by the time that is
    // passing each frame.
    if (accumulated_time_ < fadein_time_) {
      // Calculate the percentage that the screen should be faded in based on
      // the accumulated time.
      fade_percentage_ = accumulated_time_ / fadein_time_;
    } else {
      // If the fade in time is complete then turn off the fade effect and
