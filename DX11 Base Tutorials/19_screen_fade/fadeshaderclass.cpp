...
bool FadeShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"fade.hlsl", L"fade.hlsl");
  if (!result) {
    return false;
  }
...
void FadeShaderClass::Shutdown() { ShutdownShader(); }

bool FadeShaderClass::Render(int indexCount, const XMMATRIX &worldMatrix,
...
bool FadeShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                       WCHAR *psFilename) {

  ID3D10Blob *errorMessage = nullptr;
  ID3D10Blob *vertexShaderBuffer = nullptr;

  auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "FadeVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "FadePixelShader",
...
void FadeShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                               HWND hwnd,
...
bool FadeShaderClass::SetShaderParameters(const XMMATRIX &worldMatrix,
                                          const XMMATRIX &viewMatrix,
                                          const XMMATRIX &projectionMatrix,
                                          ID3D11ShaderResourceView *texture,
                                          float fadeAmount) {
  ...
  result = device_context->Map(m_fadeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                               &mappedResource);
  if (FAILED(result)) {
    return false;
  }
