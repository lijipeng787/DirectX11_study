...
bool HorizontalBlurShaderClass::Initialize(HWND hwnd) {
	auto result = InitializeShader(hwnd, L"../../3_blur_demo/horizontal_blur.hlsl", L"../../3_blur_demo/horizontal_blur.hlsl");
	if (!result) {
		return false;
	}
...
bool HorizontalBlurShaderClass::Render(int indexCount,
									   ...
									   float screenWidth) {
	auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, screenWidth);
	if (!result) {
		return false;
	}
...
bool HorizontalBlurShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "HorizontalBlurVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
