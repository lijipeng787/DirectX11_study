...
bool VerticalBlurShaderClass::Initialize(HWND hwnd) {
	...
	auto result = InitializeShader(hwnd, L"../../3_blur_demo/vertical_blur.hlsl", L"../../3_blur_demo/vertical_blur.hlsl");
	if (!result) {
		return false;
	}
...
bool VerticalBlurShaderClass::Render(int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	...
	auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, screenHeight);
	if (!result) {
		return false;
	}
...
bool VerticalBlurShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "VerticalBlurVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
