...
bool SsaoShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  error = wcscpy_s(vsFilename, 128, L"./ssao.vs");
  if (error != 0) {
    return false;
  }

  // Set the filename of the pixel shader.
  error = wcscpy_s(psFilename, 128, L"./ssao.ps");
  if (error != 0) {
    return false;
  }
...
bool SsaoShaderClass::Render(ID3D11DeviceContext *deviceContext, int indexCount,
                             ...
                             float ssaoScale, float ssaoBias,
                             float ssaoIntensity) {
  bool result;

  // Set the shader parameters that it will use for rendering.
  result = SetShaderParameters(
      deviceContext, worldMatrix, viewMatrix, projectionMatrix, positionTexture,
      normalTexture, randomTexture, screenWidth, screenHeight,
...
bool SsaoShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "SsaoVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "SsaoPixelShader",
...
void SsaoShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                               HWND hwnd,
...
bool SsaoShaderClass::SetShaderParameters(
    ID3D11DeviceContext *deviceContext, XMMATRIX worldMatrix,
    XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
    ID3D11ShaderResourceView *positionTexture,
    ID3D11ShaderResourceView *normalTexture,
    ID3D11ShaderResourceView *randomTexture, float screenWidth,
    float screenHeight, float randomTextureSize, float sampleRadius,
  ...
  result = deviceContext->Map(m_ssaoBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Now set the ssao constant buffer in the pixel shader with the updated
  // values.
...
void SsaoShaderClass::RenderShader(ID3D11DeviceContext *deviceContext,
                                   int indexCount) {
