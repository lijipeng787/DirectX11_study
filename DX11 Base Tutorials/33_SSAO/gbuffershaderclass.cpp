...
bool GBufferShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  error = wcscpy_s(vsFilename, 128, L"./gbuffer.vs");
  if (error != 0) {
    return false;
  }

  // Set the filename of the pixel shader.
  error = wcscpy_s(psFilename, 128, L"./gbuffer.ps");
  if (error != 0) {
    return false;
  }
...
bool GBufferShaderClass::Render(ID3D11DeviceContext *deviceContext,
...
bool GBufferShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
                                          WCHAR *vsFilename,
                                          WCHAR *psFilename) {
  HRESULT result;
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "GBufferVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "GBufferPixelShader",
...
void GBufferShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                  HWND hwnd,
...
bool GBufferShaderClass::SetShaderParameters(
...
void GBufferShaderClass::RenderShader(ID3D11DeviceContext *deviceContext,
                                      int indexCount) {
