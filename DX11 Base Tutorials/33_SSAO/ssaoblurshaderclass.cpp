...
bool SsaoBlurShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  error = wcscpy_s(vsFilename, 128, L"./ssaoblur.vs");
  if (error != 0) {
    return false;
  }

  // Set the filename of the pixel shader.
  error = wcscpy_s(psFilename, 128, L"./ssaoblur.ps");
  if (error != 0) {
    return false;
  }
...
bool SsaoBlurShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result = SetShaderParameters(
      deviceContext, worldMatrix, viewMatrix, projectionMatrix, ssaoTexture,
      normalDepthTexture, screenWidth, screenHeight, blurType);
  if (!result) {
    return false;
  }
...
bool SsaoBlurShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
                                           WCHAR *vsFilename,
                                           WCHAR *psFilename) {
  HRESULT result;
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "SsaoBlurVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "SsaoBlurPixelShader",
  ...
  // Setup the description of the dynamic pixel constant buffer that is in the
  // pixel shader.
...
void SsaoBlurShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                   HWND hwnd,
...
bool SsaoBlurShaderClass::SetShaderParameters(
    ID3D11DeviceContext *deviceContext, XMMATRIX worldMatrix,
    XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
    ID3D11ShaderResourceView *ssaoTexture,
    ID3D11ShaderResourceView *normalDepthTexture, int screenWidth,
    int screenHeight, int blurType) {
  HRESULT result;
  ...
  result = deviceContext->Map(m_screenBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Now set the pixel constant buffer in the pixel shader with the updated
  // value.
...
void SsaoBlurShaderClass::RenderShader(ID3D11DeviceContext *deviceContext,
                                       int indexCount) {
