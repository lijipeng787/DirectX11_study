...
void ParticleSystemClass::EmitParticles(float frameTime) {
	...
	if ((emitParticle == true) && (current_particle_count_ < (max_particles_ - 1))) {
		...
		positionX = (((float)rand() - (float)rand()) / RAND_MAX) * particle_deviatio_x_;
		positionY = (((float)rand() - (float)rand()) / RAND_MAX) * particle_deviation_y_;
		positionZ = (((float)rand() - (float)rand()) / RAND_MAX) * particle_deviation_z_;
		...
		found = false;
		while (!found) {
			if ((particle_list_[index].active == false) || (particle_list_[index].positionZ < positionZ)) {
				found = true;
			}
			else {
				index++;
...
void ParticleSystemClass::UpdateParticles(float frameTime) {
	...
	for (int i = 0; i < current_particle_count_; i++) {
		particle_list_[i].positionY = particle_list_[i].positionY - (particle_list_[i].velocity * frameTime * 0.001f);
	}
}
...
void ParticleSystemClass::KillParticles() {
	...
	for (int i = 0; i < max_particles_; i++) {
		if ((particle_list_[i].active == true) && (particle_list_[i].positionY < -3.0f)) {
			particle_list_[i].active = false;
			current_particle_count_--;
...
bool ParticleSystemClass::UpdateBuffers() {
	...
	for (int i = 0; i < current_particle_count_; i++) {
		// Bottom left.
		vertices_[index].position = XMFLOAT3(particle_list_[i].positionX - particle_size_, particle_list_[i].positionY - particle_size_, particle_list_[i].positionZ);
		vertices_[index].texture = XMFLOAT2(0.0f, 1.0f);
		vertices_[index].color = XMFLOAT4(particle_list_[i].red, particle_list_[i].green, particle_list_[i].blue, 1.0f);
		index++;
