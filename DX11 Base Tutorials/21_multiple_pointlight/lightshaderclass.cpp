...
bool LightShaderClass::Render(
	...
	auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, diffuseColor, lightPosition);
	if (!result) {
		return false;
	}
...
bool LightShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	auto result = D3DCompileFromFile(
		vsFilename, NULL, NULL, "LightVertexShader",
	...
	result = D3DCompileFromFile(
		psFilename, NULL, NULL, "LightPixelShader",
...
bool LightShaderClass::SetShaderParameters(
	...
	result = device_context->Map(m_lightColorBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
