...
bool TranslateShaderClass::Initialize(HWND hwnd) {
  ...
  auto result = InitializeShader(hwnd, L"texture_translation.hlsl",
                                 L"texture_translation.hlsl");
  if (!result) {
    return false;
  }
...
void TranslateShaderClass::Shutdown() { ShutdownShader(); }
...
bool TranslateShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                            WCHAR *psFilename) {
  ...
  auto result = D3DCompileFromFile(
      vsFilename, NULL, NULL, "TranslateVertexShader", "vs_5_0",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "TranslatePixelShader",
...
void TranslateShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                    HWND hwnd,
...
bool TranslateShaderClass::SetShaderParameters(
  ...
  result = device_context->Map(m_translateBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                               &mappedResource);
  if (FAILED(result)) {
    return false;
  }
