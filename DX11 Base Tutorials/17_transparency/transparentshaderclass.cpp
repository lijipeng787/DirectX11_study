...
bool TransparentShaderClass::Initialize(HWND hwnd) {
  ...
  auto result =
      InitializeShader(hwnd, L"transparent.hlsl", L"transparent.hlsl");
  if (!result) {
    return false;
  }
...
void TransparentShaderClass::Shutdown() { ShutdownShader(); }
...
bool TransparentShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
                                              WCHAR *psFilename) {
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "TransparentPixelShader",
  ...
  result =
      device->CreateBuffer(&transparentBufferDesc, NULL, &transparent_buffer_);
  if (FAILED(result)) {
    return false;
  }
...
void TransparentShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                      HWND hwnd,
...
bool TransparentShaderClass::SetShaderParameters(
