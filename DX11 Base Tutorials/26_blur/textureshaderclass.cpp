...
bool TextureShaderClass::InitializeShader(HWND hwnd, WCHAR *vsFilename,
  ...
  auto result =
      D3DCompileFromFile(vsFilename, NULL, NULL, "TextureVertexShader",
                         "vs_5_0", flag, 0, &vertexShaderBuffer, &errorMessage);
  if (FAILED(result)) {
    if (errorMessage) {
  ...
  result =
      D3DCompileFromFile(psFilename, NULL, NULL, "TexturePixelShader", "ps_5_0",
