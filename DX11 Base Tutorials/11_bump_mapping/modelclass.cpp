...
bool ModelClass::Initialize(char *modelFilename, WCHAR *textureFilename1,
...
void ModelClass::CalculateNormal(VectorType *tangent, VectorType *binormal,
                                 VectorType &normal) {

  // Calculate the cross product of the tangent and binormal which will give the
  // normal vector.
  ...
  auto length = sqrt((normal.x * normal.x) + (normal.y * normal.y) +
                     (normal.z * normal.z));
