...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
	...
	if (!(directx11_device_->Initialize(screenWidth, screenHeight, VSYNC_ENABLED, hwnd, FULL_SCREEN, SCREEN_DEPTH, SCREEN_NEAR))) {
...
void GraphicsClass::Frame(float deltatime) {
	...
	if (!scene_->GetRenderGroups().empty()) {
		...
		auto renderGroup = std::dynamic_pointer_cast<StandardRenderGroup>(scene_->GetRenderGroups()[0]);
