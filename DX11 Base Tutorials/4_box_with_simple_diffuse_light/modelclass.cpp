...
bool ModelClass::Initialize(const std::string & modelFilename, const std::wstring & textureFilename) {
	if (!LoadModel(modelFilename) || !InitializeBuffers() || !LoadTexture(textureFilename)) {
		return false;
	}
	return true;
}
...
void ModelClass::Render(const IShader& shader, const DirectX::XMMATRIX& viewMatrix, const DirectX::XMMATRIX& projectionMatrix) const {
	RenderBuffers();
...
DirectX::XMMATRIX ModelClass::GetWorldMatrix() const {
...
bool ModelClass::InitializeBuffers() {
	...
	for (int i = 0; i < vertex_count_; i++) {
		vertices[i].position = DirectX::XMFLOAT3(model_[i].x, model_[i].y, model_[i].z);
		vertices[i].texture = DirectX::XMFLOAT2(model_[i].tu, model_[i].tv);
		vertices[i].normal = DirectX::XMFLOAT3(model_[i].nx, model_[i].ny, model_[i].nz);
		indices[i] = i;
	}
	...
	if (FAILED(device->CreateBuffer(&vertexBufferDesc, &vertexData, &vertex_buffer_))) {
		return false;
	}
	...
	if (FAILED(device->CreateBuffer(&indexBufferDesc, &indexData, &index_buffer_))) {
		return false;
	}
