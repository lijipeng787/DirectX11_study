...
public:
    const std::vector<std::shared_ptr<IRenderGroup>>& GetRenderGroups() const { return renderGroups_; }

    std::shared_ptr<Camera> GetCamera() const { return camera_; }

    const std::vector<std::shared_ptr<LightClass>>& GetLights() const { return lights_; }
