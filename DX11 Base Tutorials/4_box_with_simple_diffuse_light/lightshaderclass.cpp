...
bool LightShaderClass::SetShaderParameters(const DirectX::XMMATRIX& worldMatrix,
	const DirectX::XMMATRIX& viewMatrix,
	const DirectX::XMMATRIX& projectionMatrix,
	ID3D11ShaderResourceView* texture,
	const DirectX::XMFLOAT3& lightDirection,
	...
	HRESULT result = deviceContext->Map(resources_.matrix_buffer_.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
	...
	deviceContext->VSSetConstantBuffers(0, 1, resources_.matrix_buffer_.GetAddressOf());

	// Set the texture
	deviceContext->PSSetShaderResources(0, 1, &texture);

	// Set the light buffer
	result = deviceContext->Map(resources_.light_buffer_.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
	...
	deviceContext->PSSetConstantBuffers(0, 1, resources_.light_buffer_.GetAddressOf());

	return true;
}

bool LightShaderClass::InitializeShader(HWND hwnd, const std::wstring& vsFilename, const std::wstring& psFilename) {
	ComPtr<ID3D10Blob> errorMessage;
	ComPtr<ID3D10Blob> vertexShaderBuffer;
	ComPtr<ID3D10Blob> pixelShaderBuffer;

	// Compile the vertex shader
	HRESULT result = D3DCompileFromFile(vsFilename.c_str(), nullptr, nullptr, "LightVertexShader", "vs_5_0",
	...
	result = D3DCompileFromFile(psFilename.c_str(), nullptr, nullptr, "LightPixelShader", "ps_5_0",
	...
	result = device->CreateVertexShader(vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(),
		nullptr, resources_.vertex_shader_.GetAddressOf());
	if (FAILED(result)) {
		return false;
	}
	...
	result = device->CreateSamplerState(&samplerDesc, resources_.sampler_state_.GetAddressOf());
	if (FAILED(result)) {
		return false;
	}
	...
	result = device->CreateBuffer(&matrixBufferDesc, nullptr, resources_.matrix_buffer_.GetAddressOf());
	if (FAILED(result)) {
		return false;
	}
	...
	result = device->CreateBuffer(&lightBufferDesc, nullptr, resources_.light_buffer_.GetAddressOf());
	if (FAILED(result)) {
		return false;
	}
...
void LightShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const std::wstring& shaderFilename) {
