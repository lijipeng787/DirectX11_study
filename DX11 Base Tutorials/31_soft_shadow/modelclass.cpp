...
bool PBRModelClass::Initialize(const char *modelFilename,
...
void PBRModelClass::Shutdown() { ReleaseTextures(); }

void PBRModelClass::Render(
    const IShader &shader,
    const ShaderParameterContainer &parameterContainer) const {
...
ID3D11ShaderResourceView *PBRModelClass::GetTexture(int index) {
  return m_Textures[index].GetTexture();
}

bool PBRModelClass::InitializeBuffers() {
...
void PBRModelClass::SetParameterCallback(ShaderParameterCallback callback) {}

ShaderParameterCallback PBRModelClass::GetParameterCallback() const {
  return [this](ShaderParameterContainer &params) { assert(0); };
}

void PBRModelClass::RenderBuffers() const {
...
bool PBRModelClass::LoadTextures(const string &filename1,
                                 const string &filename2,
                                 const string &filename3) {

  m_Textures = new TGATextureClass[3];
...
bool PBRModelClass::LoadModel(const char *filename) {
  ifstream fin;
  char input;
...
void PBRModelClass::CalculateModelVectors() {
...
void PBRModelClass::CalculateTangentBinormal(TempVertexType vertex1,
