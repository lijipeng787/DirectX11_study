...
inline std::wstring GetWString(const nlohmann::json &j,
...
PBRModelConfig ParsePBRModelConfig(const nlohmann::json &j,
                                   const std::string &name) {
  if (!j.is_object())
    return PBRModelConfig();

  return PBRModelConfig(name, j.value("model_path", ""),
                        j.value("albedo_path", ""), j.value("normal_path", ""),
                        j.value("roughmetal_path", ""));
}

// Parse render target configuration
RenderTargetConfig ParseRenderTargetConfig(const nlohmann::json &j,
                                           const std::string &name) {
  if (!j.is_object())
    return RenderTargetConfig();

  return RenderTargetConfig(name, j.value("width", 0), j.value("height", 0),
                            j.value("depth", 0.0f), j.value("near", 0.0f));
}

// Parse ortho window configuration
OrthoWindowConfig ParseOrthoWindowConfig(const nlohmann::json &j,
                                         const std::string &name) {
  if (!j.is_object())
    return OrthoWindowConfig();

  return OrthoWindowConfig(name, j.value("width", 0), j.value("height", 0));
}
} // namespace
    ...
    if (j.find("models") != j.end() && j["models"].is_object()) {
      ...
      if (models.find("refraction") != models.end() &&
        ...
        if (ref.find("ground") != ref.end() && ref["ground"].is_object()) {
          config.refraction.ground =
              ParseModelConfig(ref["ground"], "refraction_ground");
        }

        if (ref.find("wall") != ref.end() && ref["wall"].is_object()) {
          config.refraction.wall =
              ParseModelConfig(ref["wall"], "refraction_wall");
        }

        if (ref.find("water") != ref.end() && ref["water"].is_object()) {
          config.refraction.water =
              ParseModelConfig(ref["water"], "refraction_water");
        }
      }
    }
