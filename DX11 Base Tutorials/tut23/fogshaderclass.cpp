...
bool FogShaderClass::Initialize(HWND hwnd) {
	...
	auto result = InitializeShader(hwnd, L"../../tut23/fog.hlsl", L"../../tut23/fog.hlsl");
	if (!result) {
		return false;
	}
...
bool FogShaderClass::Render(int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	...
	auto result = SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, fogStart, fogEnd);
	if (!result) {
		return false;
	}
...
bool FogShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	auto result = D3DCompileFromFile(vsFilename, NULL, NULL, "FogVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
bool FogShaderClass::SetShaderParameters(const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	...
	result = device_context->Map(fog_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
