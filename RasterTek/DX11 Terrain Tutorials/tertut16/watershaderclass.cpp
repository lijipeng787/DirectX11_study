...
bool WaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../../tertut16/water.vs", L"../../tertut16/water.ps");
	if(!result)
	{
		return false;
	}
...
bool WaterShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, 
							  ...
							  ID3D11ShaderResourceView* reflectionTexture, ID3D11ShaderResourceView* normalTexture, const XMFLOAT3&cameraPosition, 
							  const XMFLOAT2& normalMapTiling, float waterTranslation, float reflectRefractScale, const XMFLOAT4&refractionTint, 
...
bool WaterShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix,
										   ...
										   const XMFLOAT3& cameraPosition, const XMFLOAT2& normalMapTiling, float waterTranslation, float reflectRefractScale,
										   const XMFLOAT4& refractionTint, const XMFLOAT3&lightDirection, float specularShininess )
{
	HRESULT result;
