...
public:
	...
	~D3D12Device() { WaitForPreviousFrame(); }
...
public:
	...
	bool render(
		...
		ID3D12DescriptorHeap * const descriptor_arr[],
		const UINT num_descriptors,
		const D3d12ResourcePtr constant_buffer1,
		const D3d12ResourcePtr constant_buffer2
		);

	bool WaitForPreviousFrame();

	void GetProjectionMatrix(DirectX::XMMATRIX& projection) { projection = projection_matrix_; }

	void GetWorldMatrix(DirectX::XMMATRIX& world) { world = world_matrix_; }

	void GetOrthoMatrix(DirectX::XMMATRIX& ortho) { ortho = ortho_matrix_; }
