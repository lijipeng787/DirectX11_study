...
bool Model::UpdateMatrixConstant(
	...
	range.End = 0;
	UINT8 *data_begin = 0;
	if (FAILED(matrix_constant_->Map(0, &range, reinterpret_cast<void**>(&data_begin)))) {
		return false;
	}
	else {
		...
		matrix_constant_->Unmap(0, nullptr);
	}
...
bool Model::UpdateLightConstant(
	...
	range.End = 0;
	UINT8 *data_begin = 0;
	if (FAILED(light_constant_->Map(0, &range, reinterpret_cast<void**>(&data_begin)))) {
		return false;
	}
	else {
		light_constant_data_.color_ = color;
		...
		light_constant_->Unmap(0, nullptr);
	}
