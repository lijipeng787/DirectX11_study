...
bool Model::Initialize(WCHAR* model_filename, WCHAR* texture_filename) {
...
bool Model::InitializeBuffers() {
	...
	ZeroMemory(&camera_constant_data, sizeof(CameraBufferType));
	...
	ZeroMemory(&light_constant_data_, sizeof(LightType));
...
bool Model::InitializeRootSignature() {
	...
	root_parameters[2].InitAsConstantBufferView(1, 0, D3D12_SHADER_VISIBILITY_VERTEX);
	root_parameters[3].InitAsConstantBufferView(0, 0, D3D12_SHADER_VISIBILITY_PIXEL);
...
D3d12ResourcePtr Model::GetMatrixConstantBuffer() const {
	return matrix_constant_buffer_;
}

D3d12ResourcePtr Model::GetLightConstantBuffer() const {
	return light_constant_buffer_;
}

D3d12ResourcePtr Model::GetCameraConstantBuffer() const{
...
bool Model::UpdateCameraConstant(DirectX::XMFLOAT3 & camera_position) {

	D3D12_RANGE range;
	range.Begin = 0;
	range.End = 0;
	UINT8 *data_begin = 0;
	if (FAILED(camera_constant_buffer_->Map(0, &range, reinterpret_cast<void**>(&data_begin)))) {
		return false;
	}
	else {
		camera_constant_data.camera_position_ = camera_position;
		camera_constant_data.padding = 0.0f;
		memcpy(data_begin, &camera_constant_data, sizeof(CameraBufferType));
