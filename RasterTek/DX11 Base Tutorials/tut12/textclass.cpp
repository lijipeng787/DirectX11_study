...
bool Text::Initialize(int screenWidth, int screenHeight, const DirectX::XMMATRIX& baseViewMatrix) {
	...
	m_base_view_matrix = baseViewMatrix;
...
bool Text::UpdateSentenceVertexBuffer(
	...
	auto drawX = static_cast<float>(((m_screen_width / 2) * -1) + positionX);
	auto drawY = static_cast<float>((m_screen_height / 2) - positionY);

	// Use the font class to build the vertex array from the sentence text and sentence draw location.
	m_font_->BuildVertexArray((void*)vertices, text, drawX, drawY);

	D3d12ResourcePtr upload_vertex_buffer = nullptr;
	...
	CD3DX12_RANGE read_range(0, 0);
	if (FAILED(upload_vertex_buffer->Map(0, &read_range, reinterpret_cast<void**>(&vertex_data_begin)))) {
		return false;
	}
	memcpy(vertex_data_begin, vertices, sizeof(VertexType) * sentence->vertex_count_);
	upload_vertex_buffer->Unmap(0, nullptr);
