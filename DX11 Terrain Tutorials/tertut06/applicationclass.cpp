...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd,
  ...
  result = m_Terrain->Initialize(m_Direct3D->GetDevice(),
                                 "../tertut06/data/heightmap01.bmp",
                                 L"../tertut06/data/dirt01.dds");
  if (!result) {
  ...
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the quad tree object.", L"Error",
               MB_OK);
    return false;
  }
...
bool ApplicationClass::Frame() {
  ...
  // Get the height of the triangle that is directly underneath the given camera
  // position.
  foundHeight = m_QuadTree->GetHeightAtPosition(position.x, position.z, height);
  if (foundHeight) {
    // If there was a triangle under the camera then position the camera just
    // above it by two units.
...
bool ApplicationClass::RenderGraphics() {
  ...
  result = m_TerrainShader->SetShaderParameters(
      m_Direct3D->GetDeviceContext(), worldMatrix, viewMatrix, projectionMatrix,
