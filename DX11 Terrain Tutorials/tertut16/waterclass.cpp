...
bool WaterClass::Initialize(ID3D11Device *device, WCHAR *textureFilename,
                            float waterHeight, float waterRadius) {
  bool result;
...
float WaterClass::GetSpecularShininess() { return m_specularShininess; }
...
bool WaterClass::InitializeBuffers(ID3D11Device *device, float waterRadius) {
  ...
  vertices[2].position =
      XMFLOAT3(-waterRadius, 0.0f, -waterRadius); // Bottom left.
  ...
  vertices[5].position =
      XMFLOAT3(waterRadius, 0.0f, -waterRadius); // Bottom right.
