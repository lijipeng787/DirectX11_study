...
bool WaterShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut16/water.vs",
                            L"../tertut16/water.ps");
  if (!result) {
    return false;
  }
...
bool WaterShaderClass::Render(
    ...
    ID3D11ShaderResourceView *normalTexture, const XMFLOAT3 &cameraPosition,
    const XMFLOAT2 &normalMapTiling, float waterTranslation,
    float reflectRefractScale, const XMFLOAT4 &refractionTint,
    const XMFLOAT3 &lightDirection, float specularShininess) {
  bool result;

  // Set the shader parameters that it will use for rendering.
  result = SetShaderParameters(
      deviceContext, worldMatrix, viewMatrix, projectionMatrix,
      reflectionMatrix, refractionTexture, reflectionTexture, normalTexture,
      cameraPosition, normalMapTiling, waterTranslation, reflectRefractScale,
...
bool WaterShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "WaterVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "WaterPixelShader",
  ...
  // Setup the description of the matrix dynamic constant buffer that is in the
  // vertex shader.
  ...
  // Setup the description of the camera and normal tiling dynamic constant
  // buffer that is in the vertex shader.
  ...
  // Setup the description of the water dynamic constant buffer that is in the
  // pixel shader.
...
void WaterShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                HWND hwnd,
...
bool WaterShaderClass::SetShaderParameters(
    ID3D11DeviceContext *deviceContext, const XMMATRIX &worldMatrix,
    const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix,
    const XMMATRIX &reflectionMatrix,
    ID3D11ShaderResourceView *refractionTexture,
    ID3D11ShaderResourceView *reflectionTexture,
    ID3D11ShaderResourceView *normalTexture, const XMFLOAT3 &cameraPosition,
    const XMFLOAT2 &normalMapTiling, float waterTranslation,
  ...
  dataPtr->reflection = reflectionMatrixCopy;
  ...
  // Now set the matrix constant buffer in the vertex shader with the updated
  // values.
  deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_matrixBuffer);

  // Lock the camera and normal tiling constant buffer so it can be written to.
  result = deviceContext->Map(m_camNormBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  result = deviceContext->Map(m_waterBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Finally set the water constant buffer in the pixel shader with the updated
  // values.
