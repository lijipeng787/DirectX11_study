...
bool TerrainShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut16/terrain.vs",
                            L"../tertut16/terrain.ps");
  if (!result) {
    return false;
  }
...
bool TerrainShaderClass::Render(
  ...
  result = SetShaderParameters(
      deviceContext, worldMatrix, viewMatrix, projectionMatrix, colorTexture,
      normalTexture, lightDiffuseColor, lightDirection, colorTextureBrightness);
  if (!result) {
    return false;
  }
