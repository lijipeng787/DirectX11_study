...
bool ReflectionShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut16/reflection.vs",
                            L"../tertut16/reflection.ps");
  if (!result) {
    return false;
  }
...
bool ReflectionShaderClass::Render(
    ...
    ID3D11ShaderResourceView *normalTexture, const XMFLOAT4 &lightDiffuseColor,
    const XMFLOAT3 &lightDirection, float colorTextureBrightness,
  ...
  result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
                               ...
                               lightDiffuseColor, lightDirection,
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "ReflectionVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "ReflectionPixelShader",
  ...
  // Setup the description of the clip plane dynamic constant buffer that is in
  // the vertex shader.
...
void ReflectionShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                     HWND hwnd,
...
bool ReflectionShaderClass::SetShaderParameters(
  ...
  result = deviceContext->Map(m_clipPlaneBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Now set the clip plane constant buffer in the vertex shader with the
  // updated values.
  ...
  // Finally set the light constant buffer in the pixel shader with the updated
  // values.
