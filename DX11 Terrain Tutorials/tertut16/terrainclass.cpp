...
bool TerrainClass::Initialize(ID3D11Device *device, char *heightMapFilename,
                              char *colorMapFilename, float flattenAmount,
  ...
  // Release the height map and the model since the data is now loaded into the
...
bool TerrainClass::InitializeBuffers(ID3D11Device *device) {
  ...
  for (i = 0; i < m_vertexCount; i++) {
    ...
    vertices[i].color =
        XMFLOAT4(m_model[i].r, m_model[i].g, m_model[i].b, 1.0f);
