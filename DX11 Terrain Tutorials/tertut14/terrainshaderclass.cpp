...
bool TerrainShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut14/terrain.vs",
                            L"../tertut14/terrain.ps");
  if (!result) {
    return false;
  }
...
bool TerrainShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result = SetShaderParameters(
      deviceContext, worldMatrix, viewMatrix, projectionMatrix, ambientColor,
      diffuseColor, lightDirection, grassTexture, slopeTexture, rockTexture);
  if (!result) {
    return false;
  }
...
bool TerrainShaderClass::SetShaderParameters(
    ...
    const XMFLOAT3 &lightDirection, ID3D11ShaderResourceView *grassTexture,
    ID3D11ShaderResourceView *slopeTexture,
    ID3D11ShaderResourceView *rockTexture) {
  HRESULT result;
