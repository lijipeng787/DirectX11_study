...
bool SkyPlaneShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut12/skyplane.vs",
                            L"../tertut12/skyplane.ps");
  if (!result) {
    return false;
  }
...
bool SkyPlaneShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
                               projectionMatrix, cloudTexture, perturbTexture,
...
bool SkyPlaneShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
  ...
  // Setup the description of the dynamic sky constant buffer that is in the
  // pixel shader.
  ...
  // Create the sky buffer pointer so we can access the pixel shader constant
...
bool SkyPlaneShaderClass::SetShaderParameters(
  ...
  // Now set the sky constant buffer in the pixel shader with the updated
  // values.
