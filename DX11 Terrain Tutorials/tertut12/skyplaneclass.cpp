...
bool SkyPlaneClass::Initialize(ID3D11Device *device,
                               WCHAR *cloudTextureFilename,
                               WCHAR *perturbTextureFilename) {
  ...
  result = InitializeSkyPlane(skyPlaneResolution, skyPlaneWidth, skyPlaneTop,
                              skyPlaneBottom, textureRepeat);
  if (!result) {
    return false;
  }
...
void SkyPlaneClass::Shutdown() {
  ...
  // Release the vertex and index buffer that were used for rendering the sky
  // plane.
  ShutdownBuffers();
...
bool SkyPlaneClass::InitializeSkyPlane(int skyPlaneResolution,
                                       float skyPlaneWidth, float skyPlaneTop,
  ...
  m_skyPlane =
      new SkyPlaneType[(skyPlaneResolution + 1) * (skyPlaneResolution + 1)];
  if (!m_skyPlane) {
    return false;
  }
  ...
  // Loop through the sky plane and build the coordinates based on the increment
  // values given.
  for (j = 0; j <= skyPlaneResolution; j++) {
    for (i = 0; i <= skyPlaneResolution; i++) {
      ...
      positionY =
          skyPlaneTop -
          (constant * ((positionX * positionX) + (positionZ * positionZ)));
...
bool SkyPlaneClass::InitializeBuffers(ID3D11Device *device,
  ...
  for (j = 0; j < skyPlaneResolution; j++) {
    for (i = 0; i < skyPlaneResolution; i++) {
      ...
      vertices[index].position = XMFLOAT3(
          m_skyPlane[index1].x, m_skyPlane[index1].y, m_skyPlane[index1].z);
      vertices[index].texture =
          XMFLOAT2(m_skyPlane[index1].tu, m_skyPlane[index1].tv);
      indices[index] = index;
      index++;

      // Triangle 1 - Upper Right
      vertices[index].position = XMFLOAT3(
          m_skyPlane[index2].x, m_skyPlane[index2].y, m_skyPlane[index2].z);
      vertices[index].texture =
          XMFLOAT2(m_skyPlane[index2].tu, m_skyPlane[index2].tv);
      indices[index] = index;
      index++;

      // Triangle 1 - Bottom Left
      vertices[index].position = XMFLOAT3(
          m_skyPlane[index3].x, m_skyPlane[index3].y, m_skyPlane[index3].z);
      vertices[index].texture =
          XMFLOAT2(m_skyPlane[index3].tu, m_skyPlane[index3].tv);
      indices[index] = index;
      index++;
      ...
      vertices[index].position = XMFLOAT3(
          m_skyPlane[index4].x, m_skyPlane[index4].y, m_skyPlane[index4].z);
      vertices[index].texture =
          XMFLOAT2(m_skyPlane[index4].tu, m_skyPlane[index4].tv);
      indices[index] = index;
      index++;
    }
  }
...
bool SkyPlaneClass::LoadTextures(ID3D11Device *device, WCHAR *textureFilename1,
