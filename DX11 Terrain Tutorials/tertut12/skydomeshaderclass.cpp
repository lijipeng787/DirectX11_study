...
bool SkyDomeShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut12/skydome.vs",
                            L"../tertut12/skydome.ps");
  if (!result) {
    return false;
  }
...
bool SkyDomeShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
                               projectionMatrix, apexColor, centerColor);
  if (!result) {
    return false;
  }
...
bool SkyDomeShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
                                          WCHAR *vsFilename,
                                          WCHAR *psFilename) {
  HRESULT result;
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "SkyDomeVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "SkyDomePixelShader",
  ...
  // Setup the description of the gradient constant buffer that is in the pixel
  // shader.
...
bool SkyDomeShaderClass::SetShaderParameters(ID3D11DeviceContext *deviceContext,
  ...
  result = deviceContext->Map(m_gradientBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Finally set the gradient constant buffer in the pixel shader with the
  // updated values.
...
void SkyDomeShaderClass::RenderShader(ID3D11DeviceContext *deviceContext,
                                      int indexCount) {
