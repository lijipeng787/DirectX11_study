...
bool TerrainShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut09/terrain.vs",
                            L"../tertut09/terrain.ps");
  if (!result) {
    return false;
  }
...
bool TerrainShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
                                          WCHAR *vsFilename,
                                          WCHAR *psFilename) {
  HRESULT result;
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "TerrainVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "TerrainPixelShader",
  ...
  // Setup the description of the texture info constant buffer that is in the
  // pixel shader.
  ...
  // constant buffer from within this class.
  result =
      device->CreateBuffer(&textureInfoBufferDesc, NULL, &m_textureInfoBuffer);
  if (FAILED(result)) {
    return false;
  }
...
bool TerrainShaderClass::SetShaderTextures(ID3D11DeviceContext *deviceContext,
  ...
  // If this is a blended polygon then also set the second texture and the alpha
  // map for blending.
  if (useAlpha) {
  ...
  result = deviceContext->Map(m_textureInfoBuffer, 0, D3D11_MAP_WRITE_DISCARD,
                              0, &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Finally set the texture info constant buffer in the pixel shader with the
  // updated values.
