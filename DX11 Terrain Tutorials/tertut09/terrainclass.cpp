...
bool TerrainClass::CalculateNormals() {
  ...
  float vertex1[3], vertex2[3], vertex3[3], vector1[3], vector2[3], sum[3],
      length;
  VectorType *normals;
  ...
  for (j = 0; j < (m_terrainHeight - 1); j++) {
    for (i = 0; i < (m_terrainWidth - 1); i++) {
      ...
      // un-normalized value for this face normal.
  ...
  for (j = 0; j < m_terrainHeight; j++) {
    for (i = 0; i < m_terrainWidth; i++) {
      ...
      // Normalize the final shared normal for this vertex and store it in the
      // height map array.
...
bool TerrainClass::LoadColorMap(char *filename) {
  ...
  if ((colorMapWidth != m_terrainWidth) ||
      (colorMapHeight != m_terrainHeight)) {
    return false;
  }
...
bool TerrainClass::LoadMaterialFile(char *filename, char *materialMapFilename,
                                    ID3D11Device *device) {
  ifstream fin;
  char input;
  int i, error;
  ...
  for (i = 0; i < m_materialCount; i++) {
    ...
    fin >> m_Materials[i].textureIndex1 >> m_Materials[i].textureIndex2 >>
        m_Materials[i].alphaIndex;
...
bool TerrainClass::LoadMaterialMap(char *filename) {
  ...
  // Make sure the material index map dimensions are the same as the terrain
  // dimensions for 1 to 1 mapping.
  ...
  // Initialize the position in the image data buffer so each vertice has an
  // material index associated with it.
  k = 0;
...
bool TerrainClass::LoadMaterialBuffers(ID3D11Device *device) {
  int maxVertexCount, maxIndexCount, i, j, index1, index2, index3, index4,
      redIndex, greenIndex, blueIndex, index, vIndex;
  bool found;
  D3D11_BUFFER_DESC vertexBufferDesc, indexBufferDesc;
  D3D11_SUBRESOURCE_DATA vertexData, indexData;
  HRESULT result;

  // Create the value for the maximum number of vertices a material group could
  // possibly have.
  ...
  // Initialize vertex and index arrays for each material group to the maximum
  // size.
  for (i = 0; i < m_materialCount; i++) {
  ...
  // Now loop through the terrain and build the vertex arrays for each material
  // group.
  for (j = 0; j < (m_terrainHeight - 1); j++) {
    for (i = 0; i < (m_terrainWidth - 1); i++) {
      ...
      found = false;
      while (!found) {
        if ((redIndex == m_Materials[index].red) &&
            (greenIndex == m_Materials[index].green) &&
      ...
      // Upper left.
      m_Materials[index].vertices[vIndex].position = XMFLOAT3(
      ...
      m_Materials[index].vertices[vIndex].normal =
          XMFLOAT3(m_heightMap[index3].nx, m_heightMap[index3].ny,
                   m_heightMap[index3].nz);
      m_Materials[index].vertices[vIndex].color =
          XMFLOAT4(m_heightMap[index3].r, m_heightMap[index3].g,
      ...
      m_Materials[index].vertices[vIndex].normal =
          XMFLOAT3(m_heightMap[index4].nx, m_heightMap[index4].ny,
                   m_heightMap[index4].nz);
      m_Materials[index].vertices[vIndex].color =
          XMFLOAT4(m_heightMap[index4].r, m_heightMap[index4].g,
      ...
      m_Materials[index].vertices[vIndex].normal =
          XMFLOAT3(m_heightMap[index1].nx, m_heightMap[index1].ny,
                   m_heightMap[index1].nz);
      m_Materials[index].vertices[vIndex].color =
          XMFLOAT4(m_heightMap[index1].r, m_heightMap[index1].g,
      ...
      m_Materials[index].vertices[vIndex].normal =
          XMFLOAT3(m_heightMap[index2].nx, m_heightMap[index2].ny,
                   m_heightMap[index2].nz);
      m_Materials[index].vertices[vIndex].color =
          XMFLOAT4(m_heightMap[index2].r, m_heightMap[index2].g,
  ...
  // Now create the vertex and index buffers from the vertex and index arrays
  // for each material group.
  for (i = 0; i < m_materialCount; i++) {
    vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
    vertexBufferDesc.ByteWidth =
        sizeof(VertexType) * m_Materials[i].vertexCount;
    ...
    indexBufferDesc.ByteWidth =
        sizeof(unsigned long) * m_Materials[i].indexCount;
...
bool TerrainClass::Render(
    ID3D11DeviceContext *deviceContext, TerrainShaderClass *shader,
    const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix,
    const XMMATRIX &projectionMatrix, const XMFLOAT4 &ambientColor,
    const XMFLOAT4 &diffuseColor, const XMFLOAT3 &lightDirection) {
  unsigned int stride;
  unsigned int offset;
  int i;
  bool result;
  ...
  result = shader->SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
  ...
  // Set the type of primitive that should be rendered from the vertex buffers,
  ...
  for (i = 0; i < m_materialCount; i++) {
    ...
    // rendered.
    deviceContext->IASetVertexBuffers(0, 1, &m_Materials[i].vertexBuffer,
                                      &stride, &offset);
    ...
    // If the material group has a valid second texture index then this is a
    // blended terrain polygon.
    if (m_Materials[i].textureIndex2 != -1) {
      result = shader->SetShaderTextures(
          deviceContext, m_Textures[m_Materials[i].textureIndex1].GetTexture(),
