...
bool TextClass::Initialize(ID3D11Device *device, FontClass *Font,
                           int screenWidth, int screenHeight, int maxLength,
                           char *text, int positionX, int positionY, float red,
                           float green, float blue, bool shadow,
  ...
  result = InitializeSentence(device, Font, text, positionX, positionY, red,
                              green, blue, deviceContext);
  if (!result) {
    return false;
  }
...
void TextClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  // Draw the sentence.
  RenderSentence(deviceContext, ShaderManager, worldMatrix, viewMatrix,
                 orthoMatrix, fontTexture);

  return;
}

bool TextClass::InitializeSentence(ID3D11Device *device, FontClass *Font,
                                   char *text, int positionX, int positionY,
  ...
  if (m_shadow) {
    result =
        device->CreateBuffer(&vertexBufferDesc, &vertexData, &m_vertexBuffer2);
    if (FAILED(result)) {
      return false;
    }

    result =
        device->CreateBuffer(&indexBufferDesc, &indexData, &m_indexBuffer2);
    if (FAILED(result)) {
      return false;
    }
  }
  ...
  result = UpdateSentence(Font, text, positionX, positionY, red, green, blue,
                          deviceContext);
  if (!result) {
    return false;
  }
...
bool TextClass::UpdateSentence(FontClass *Font, char *text, int positionX,
  ...
  result = deviceContext->Map(m_vertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // If shadowed then do the same for the second vertex buffer but offset by one
  // pixel.
  if (m_shadow) {
    memset(vertices, 0, (sizeof(VertexType) * m_vertexCount));
    ...
    result = deviceContext->Map(m_vertexBuffer2, 0, D3D11_MAP_WRITE_DISCARD, 0,
                                &mappedResource);
    if (FAILED(result)) {
      return false;
    }
...
void TextClass::RenderSentence(ID3D11DeviceContext *deviceContext,
  ...
  if (m_shadow) {
    ...
    ShaderManager->RenderFontShader(deviceContext, m_indexCount, worldMatrix,
                                    viewMatrix, orthoMatrix, fontTexture,
                                    shadowColor);
  }
