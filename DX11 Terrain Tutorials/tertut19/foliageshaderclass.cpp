...
bool FoliageShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut19/foliage.vs",
                            L"../tertut19/foliage.ps");
  if (!result) {
    return false;
  }
...
bool FoliageShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result =
      SetShaderParameters(deviceContext, viewMatrix, projectionMatrix, texture);
  if (!result) {
    return false;
  }
...
bool FoliageShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
                                          WCHAR *vsFilename,
                                          WCHAR *psFilename) {
  HRESULT result;
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "FoliageVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "FoliagePixelShader",
...
void FoliageShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                  HWND hwnd,
...
bool FoliageShaderClass::SetShaderParameters(
    ID3D11DeviceContext *deviceContext, const XMMATRIX &viewMatrix,
...
void FoliageShaderClass::RenderShader(ID3D11DeviceContext *deviceContext,
                                      int vertexCount, int instanceCount) {
