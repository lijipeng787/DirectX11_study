...
bool FontClass::Initialize(ID3D11Device *device, char *fontFilename,
                           WCHAR *textureFilename, float fontHeight,
                           int spaceSize) {
  bool result;
  ...
  for (i = 0; i < numLetters; i++) {
    ...
    if (letter == 0) {
      ...
      vertexPtr[index].position =
          XMFLOAT3((drawX + m_Font[letter].size), (drawY - m_fontHeight),
                   0.0f); // Bottom right.
      vertexPtr[index].texture = XMFLOAT2(m_Font[letter].right, 1.0f);
      index++;

      vertexPtr[index].position =
          XMFLOAT3(drawX, (drawY - m_fontHeight), 0.0f); // Bottom left.
      ...
      vertexPtr[index].position =
          XMFLOAT3(drawX + m_Font[letter].size, drawY, 0.0f); // Top right.
...
int FontClass::GetFontHeight() { return (int)m_fontHeight; }