...
bool ShaderManagerClass::Initialize(D3DClass *Direct3D, HWND hwnd) {
  ...
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the foliage shader object.",
               L"Error", MB_OK);
    return false;
  }
...
void ShaderManagerClass::RenderTextureShader(
...
void ShaderManagerClass::RenderFontShader(ID3D11DeviceContext *deviceContext,
                                          int indexCount,
                                          ...
                                          XMFLOAT4 &color) {
  m_FontShader->Render(deviceContext, indexCount, worldMatrix, viewMatrix,
                       projectionMatrix, texture, color);
  return;
}

void ShaderManagerClass::RenderFoliageShader(
    ID3D11DeviceContext *deviceContext, int vertexCount, int instanceCount,
    const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix,
    ID3D11ShaderResourceView *texture) {
  m_FoliageShader->Render(deviceContext, vertexCount, instanceCount, viewMatrix,
                          projectionMatrix, texture);
  return;
}