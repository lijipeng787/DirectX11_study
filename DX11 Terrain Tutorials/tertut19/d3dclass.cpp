...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync,
  ...
  result =
      m_device->CreateRasterizerState(&rasterDesc, &m_rasterStateNoCulling);
  if (FAILED(result)) {
    return false;
  }
  ...
  // rendering.  The only difference is that DepthEnable is set to false, all
  // other parameters are the same as the other depth stencil state.
...
void D3DClass::TurnOffAlphaBlending() {
  ...
  m_deviceContext->OMSetBlendState(m_alphaDisableBlendingState, blendFactor,
                                   0xffffffff);
...
void D3DClass::EnableAlphaToCoverageBlending() {
  ...
  // Turn on the alpha blending.
  m_deviceContext->OMSetBlendState(m_alphaEnableBlendingState2, blendFactor,
                                   0xffffffff);
