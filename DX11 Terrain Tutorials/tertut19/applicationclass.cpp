...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd,
  ...
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the shader manager object.",
               L"Error", MB_OK);
    return false;
  }
  ...
  // Set the initial position of the camera and build the matrices needed for
  // rendering.
  ...
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the user interface object.",
               L"Error", MB_OK);
    return false;
  }
  ...
  result = m_GroundModel->Initialize(m_Direct3D->GetDevice(),
                                     "../tertut19/data/plane01.txt",
                                     L"../tertut19/data/rock015.dds");
  if (!result) {
  ...
  result = m_Foliage->Initialize(m_Direct3D->GetDevice(),
                                 L"../tertut19/data/grass.dds", 500);
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the foliage object.", L"Error",
               MB_OK);
    return false;
  }
...
bool ApplicationClass::Frame() {
  ...
  result = m_UserInterface->Frame(m_Fps->GetFps(), posX, posY, posZ, rotX, rotY,
                                  rotZ, m_Direct3D->GetDeviceContext());
  if (!result) {
    return false;
  }
...
bool ApplicationClass::Render() {
  ...
  m_ShaderManager->RenderFoliageShader(
      m_Direct3D->GetDeviceContext(), m_Foliage->GetVertexCount(),
  ...
  m_UserInterface->Render(m_Direct3D, m_ShaderManager, worldMatrix,
