...
public:
  ...
  bool Frame(int, float, float, float, float, float, float,
             ID3D11DeviceContext *);
  void Render(D3DClass *, ShaderManagerClass *, const XMMATRIX &,
              const XMMATRIX &, const XMMATRIX &);

private:
  bool UpdateFpsString(int, ID3D11DeviceContext *);
  bool UpdatePositionStrings(float, float, float, float, float, float,
                             ID3D11DeviceContext *);
