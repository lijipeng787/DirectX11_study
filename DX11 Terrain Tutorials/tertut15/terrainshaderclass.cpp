...
bool TerrainShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut15/terrain.vs",
                            L"../tertut15/terrain.ps");
  if (!result) {
    return false;
  }
...
bool TerrainShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
                               projectionMatrix, diffuseColor, lightDirection,
                               colorTexture, normalMapTexture);
  if (!result) {
    return false;
  }
