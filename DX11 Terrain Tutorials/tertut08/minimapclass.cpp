...
bool MiniMapClass::Initialize(ID3D11Device* device, HWND hwnd, int screenWidth, int screenHeight, const XMMATRIX& viewMatrix, float terrainWidth, 
							  float terrainHeight)
{
	bool result;
	...
	result = m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight, L"../../tertut08/data/colorm01.dds", 150, 150);
	if(!result)
	{
	...
	result = m_Border->Initialize(device, screenWidth, screenHeight, L"../../tertut08/data/border01.dds", 154, 154);
	if(!result)
	{
	...
	result = m_Point->Initialize(device, screenWidth, screenHeight, L"../../tertut08/data/point01.dds", 3, 3);
	if(!result)
	{
...
bool MiniMapClass::Render( ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& orthoMatrix, TextureShaderClass* textureShader )
{
	bool result;
