...
bool MiniMapClass::Initialize(ID3D11Device *device, HWND hwnd, int screenWidth,
                              int screenHeight, const XMMATRIX &viewMatrix,
  ...
  result =
      m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight,
                                  L"../tertut08/data/colorm01.dds", 150, 150);
  if (!result) {
  ...
  result = m_Border->Initialize(device, screenWidth, screenHeight,
                                L"../tertut08/data/border01.dds", 154, 154);
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the border object.", L"Error",
               MB_OK);
    return false;
  }
  ...
  result = m_Point->Initialize(device, screenWidth, screenHeight,
                               L"../tertut08/data/point01.dds", 3, 3);
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the point object.", L"Error",
               MB_OK);
    return false;
  }
...
bool MiniMapClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  // Put the border bitmap vertex and index buffers on the graphics pipeline to
  // prepare them for drawing.
  result = m_Border->Render(deviceContext, (m_mapLocationX - 2),
                            (m_mapLocationY - 2));
  if (!result) {
    return false;
  }
  ...
  textureShader->Render(deviceContext, m_Border->GetIndexCount(), worldMatrix,
                        m_viewMatrix, orthoMatrix, m_Border->GetTexture());

  // Put the mini-map bitmap vertex and index buffers on the graphics pipeline
  // to prepare them for drawing.
  result =
      m_MiniMapBitmap->Render(deviceContext, m_mapLocationX, m_mapLocationY);
  if (!result) {
    return false;
  }

  // Render the mini-map bitmap using the texture shader.
  textureShader->Render(deviceContext, m_MiniMapBitmap->GetIndexCount(),
                        worldMatrix, m_viewMatrix, orthoMatrix,
                        m_MiniMapBitmap->GetTexture());

  // Put the point bitmap vertex and index buffers on the graphics pipeline to
  // prepare them for drawing.
  ...
  textureShader->Render(deviceContext, m_Point->GetIndexCount(), worldMatrix,
                        m_viewMatrix, orthoMatrix, m_Point->GetTexture());
...
void MiniMapClass::PositionUpdate(float positionX, float positionZ) {
  ...
  // Ensure the point does not leave the minimap borders even if the camera goes
  // past the terrain borders.
  if (positionX < 0) {
    positionX = 0;
  }
  ...
  // Subtract one from the location to center the point on the mini-map
  // according to the 3x3 point pixel image size.
