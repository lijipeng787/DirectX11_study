...
bool TerrainShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut08/terrain.vs",
                            L"../tertut08/terrain.ps");
  if (!result) {
    return false;
  }
...
bool TerrainShaderClass::Render(ID3D11DeviceContext *deviceContext,
  ...
  result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix,
                               projectionMatrix, ambientColor, diffuseColor,
...
bool TerrainShaderClass::SetShaderParameters(
    ...
    const XMFLOAT3 &lightDirection, ID3D11ShaderResourceView *texture) {
  HRESULT result;
