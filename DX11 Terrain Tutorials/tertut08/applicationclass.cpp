...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd,
  ...
  result = m_Terrain->Initialize(
      m_Direct3D->GetDevice(), "../tertut08/data/heightmap01.bmp",
      L"../tertut08/data/dirt01.dds", "../tertut08/data/colorm01.bmp");
  if (!result) {
  ...
  m_MiniMap = (MiniMapClass *)_aligned_malloc(sizeof(MiniMapClass), 16);
  new (m_MiniMap) MiniMapClass();
  if (!m_MiniMap) {
    return false;
  }

  // Initialize the mini map object.
  result = m_MiniMap->Initialize(
      m_Direct3D->GetDevice(), hwnd, screenWidth, screenHeight, baseViewMatrix,
      (float)(terrainWidth - 1), (float)(terrainHeight - 1));
  if (!result) {
    MessageBox(hwnd, L"Could not initialize the mini map object.", L"Error",
               MB_OK);
    return false;
  }
...
void ApplicationClass::Shutdown() {
  ...
  if (m_MiniMap) {
    m_MiniMap->Shutdown();
    m_MiniMap->~MiniMapClass();
    _aligned_free(m_MiniMap);
    m_MiniMap = 0;
  }
...
bool ApplicationClass::RenderGraphics() {
  ...
  // Render the mini map.
  result = m_MiniMap->Render(m_Direct3D->GetDeviceContext(), worldMatrix,
                             orthoMatrix, m_TextureShader);
  if (!result) {
    return false;
  }
