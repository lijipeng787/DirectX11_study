...
bool TerrainClass::Initialize(ID3D11Device *device, char *heightMapFilename,
                              WCHAR *textureFilename, char *colorMapFilename,
                              WCHAR *detailMapFilename) {
  bool result;
...
void TerrainClass::CalculateTextureCoordinates() {
  ...
  // Loop through the entire height map and calculate the tu and tv texture
  ...
  for (j = 0; j < m_terrainHeight; j++) {
    for (i = 0; i < m_terrainWidth; i++) {
      ...
      // Check if at the far right end of the texture and if so then start at
      // the beginning again.
    ...
    // Increment the tv texture coordinate by the increment value and increment
    // the index by one.
...
bool TerrainClass::LoadTextures(ID3D11Device *device, WCHAR *filename,
                                WCHAR *detailMapFilename) {
  bool result;
  ...
  for (j = 0; j < (m_terrainHeight - 1); j++) {
    for (i = 0; i < (m_terrainWidth - 1); i++) {
      ...
      vertices[index].texture =
          XMFLOAT4(m_heightMap[index3].tu, tv, 0.0f, 0.0f);
      ...
      vertices[index].texture =
          XMFLOAT4(m_heightMap[index1].tu, m_heightMap[index1].tv, 0.0f, 1.0f);
      vertices[index].normal =
      ...
      vertices[index].texture =
          XMFLOAT4(tu, m_heightMap[index2].tv, 1.0f, 1.0f);
      vertices[index].normal =
