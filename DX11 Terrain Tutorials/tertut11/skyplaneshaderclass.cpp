...
bool SkyPlaneShaderClass::Initialize(ID3D11Device *device, HWND hwnd) {
  ...
  result = InitializeShader(device, hwnd, L"../tertut11/skyplane.vs",
                            L"../tertut11/skyplane.ps");
  if (!result) {
    return false;
  }
...
bool SkyPlaneShaderClass::Render(
    ID3D11DeviceContext *deviceContext, int indexCount,
    const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix,
    const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView *texture,
    ID3D11ShaderResourceView *texture2, float firstTranslationX,
    float firstTranslationZ, float secondTranslationX, float secondTranslationZ,
    float brightness) {
  bool result;
  ...
  result = SetShaderParameters(
      ...
      texture2, firstTranslationX, firstTranslationZ, secondTranslationX,
      secondTranslationZ, brightness);
  if (!result) {
    return false;
  }
...
bool SkyPlaneShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd,
                                           WCHAR *vsFilename,
                                           WCHAR *psFilename) {
  HRESULT result;
  ...
  result = D3DCompileFromFile(vsFilename, NULL, NULL, "SkyPlaneVertexShader",
  ...
  result = D3DCompileFromFile(psFilename, NULL, NULL, "SkyPlanePixelShader",
  ...
  // Setup the description of the sky constant buffer that is in the pixel
  // shader.
...
void SkyPlaneShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage,
                                                   HWND hwnd,
...
bool SkyPlaneShaderClass::SetShaderParameters(
  ...
  result = deviceContext->Map(m_skyBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &mappedResource);
  if (FAILED(result)) {
    return false;
  }
  ...
  // Finally set the sky constant buffer in the pixel shader with the updated
  // values.
