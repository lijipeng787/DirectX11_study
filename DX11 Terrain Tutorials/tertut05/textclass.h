...
public:
  ...
  bool Initialize(ID3D11Device *, ID3D11DeviceContext *, HWND, int, int,
                  const XMMATRIX &);
  void Shutdown();
  bool Render(ID3D11DeviceContext *, FontShaderClass *, const XMMATRIX &,
              const XMMATRIX &);
...
private:
  ...
  bool RenderSentence(SentenceType *, ID3D11DeviceContext *, FontShaderClass *,
