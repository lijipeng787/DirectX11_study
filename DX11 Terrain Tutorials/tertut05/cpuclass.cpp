...
void CpuClass::Frame() {
  ...
  if (m_canReadCpu) {
    // If it has been 1 second then update the current cpu usage and reset the 1
    // second timer again.
...
int CpuClass::GetCpuPercentage() {
  ...
  // If the class can read the cpu from the operating system then return the
  // current usage.  If not then return zero.
  if (m_canReadCpu) {
