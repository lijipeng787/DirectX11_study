...
bool TextClass::Initialize(ID3D11Device *device,
  ...
  // Store the screen width and height for calculating pixel location during the
  // sentence updates.
  ...
  result = m_Font->Initialize(device, "../tertut05/data/fontdata.txt",
                              L"../tertut05/data/font.dds");
  if (!result) {
...
bool TextClass::Render(ID3D11DeviceContext *deviceContext,
                       FontShaderClass *FontShader, const XMMATRIX &worldMatrix,
                       const XMMATRIX &orthoMatrix) {
  bool result;

  // Draw the sentences.
  result = RenderSentence(m_sentence1, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence2, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence3, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence4, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence5, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence6, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence7, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence8, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence9, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence10, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }

  result = RenderSentence(m_sentence11, deviceContext, FontShader, worldMatrix,
                          orthoMatrix);
  if (!result) {
    return false;
  }
...
bool TextClass::UpdateSentence(SentenceType *sentence, char *text,
  ...
  // Use the font class to build the vertex array from the sentence text and
  // sentence draw location.
...
bool TextClass::RenderSentence(SentenceType *sentence,
  ...
  result = FontShader->Render(deviceContext, sentence->indexCount, worldMatrix,
...
bool TextClass::SetRenderCount(int count, ID3D11DeviceContext *deviceContext) {
  ...
  // Update the sentence vertex buffer with the new string information.
  result = UpdateSentence(m_sentence11, renderString, 10, 290, 0.0f, 1.0f, 0.0f,
                          deviceContext);
  if (!result) {
    return false;
  }
